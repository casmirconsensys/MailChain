import { HEADER_LABELS, CRLF } from './consts.js';
import { buildMessageAndAttachments } from './contentHandler.js';
import { concludeHeaders } from './fallback.js';
import { hasOnlyPrintableUsAscii } from './hasOnlyAscii.js';
import { createMessageIdHeader, createHeader } from './headerFactories.js';
import { buildHeaders } from './headerHandler.js';
import { byHeaderOrder } from './headerOrder.js';
import { defaultMessageComposerContext } from './messageComposerContext.js';
import { isAddressHeader } from './types.js';
...
class MessageComposer {
    ...
    async build() {
        ...
        const contentPart = await buildMessageAndAttachments([...this._messages.values()], this._attachments, this._ctx);
        ...
        const builtHeadersForSender = await buildHeaders([...finalHeadersForSender.values()].sort(byHeaderOrder), this._ctx);
        const finalHeadersVisibleRecipients = new Map(finalHeaders);
        finalHeadersVisibleRecipients.delete(HEADER_LABELS.Bcc);
        const builtHeadersForToAndCc = await buildHeaders([...finalHeadersVisibleRecipients.values()].sort(byHeaderOrder), this._ctx);
...
function createMessageComposer(customCtx) {
    return new MessageComposer(customCtx !== null && customCtx !== void 0 ? customCtx : defaultMessageComposerContext());
}

export { MessageComposer, createMessageComposer };
