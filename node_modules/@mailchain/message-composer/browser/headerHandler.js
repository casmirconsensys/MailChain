...
import { contentAppendWithFolding, semanticLineFold, simpleHardFold } from './folding.js';
import { isStringHeader, isDateHeader, isAddressHeader, isMessageIdHeader } from './types.js';
import { CRLF, HTAB, LINE_LENGTH_FOLD } from './consts.js';
 ...
 */
async function exportDateHeader(header, ctx) {
 ...
 * ```
 */
async function exportAddressHeader(header, ctx) {
    ...
    return header.value
        .map((r) => {
        var _a;
        return Boolean((_a = r.name) === null || _a === void 0 ? void 0 : _a.length) ? `"${r.name}" <${r.address}>` : `<${r.address}>`;
    })
        .join(`,${CRLF}${HTAB}`);
}
async function exportMessageIdHeader(header, ctx) {
    return header.value.ids
        .map((id) => (id.startsWith('<') && id.endsWith('>') ? id : `<${id}>`))
        .join(`${CRLF}${HTAB}`);
}

export { buildHeaders, exportAddressHeader, exportDateHeader, exportHeader, exportHeaderAttributes, exportHeaderValue, exportMessageIdHeader, exportStringHeader };
