...
function deserializePublicKeyEncryptedContent(input) {
    ...
    switch (input[1]) {
        case keys.IdSECP256K1:
            pubKeyEnd = start + 33;
            return {
                pubKey: new _public$2.SECP256K1PublicKey(input.slice(start, pubKeyEnd)),
                encryptedContent: input.slice(pubKeyEnd),
            };
        case keys.IdED25519:
            pubKeyEnd = start + _public$1.ED25519PublicKeyLen;
            return {
                pubKey: new _public$1.ED25519PublicKey(input.slice(start, pubKeyEnd)),
                encryptedContent: input.slice(pubKeyEnd),
            };
        case keys.IdSR25519:
            pubKeyEnd = start + _public.SR25519PublicKeyLen;
            return {
                pubKey: new _public.SR25519PublicKey(input.slice(start, pubKeyEnd)),
                encryptedContent: input.slice(pubKeyEnd),
            };
        default:
