...
class DeliveryRequests {
    ...
    async processDeliveryRequest(messagingKey, delivery, deliveryRequestHash) {
        try {
            ...
            const payloadRootEncryptionKeyBytes = await messagingKey.ecdhDecrypt(crypto.decodePublicKey(ecdhKeyBundle.publicEphemeralKey), encryptedMessageKey);
            if (payloadRootEncryptionKeyBytes.length === 0) {
                throw new Error('payloadRootEncryptionKey is empty');
            }
            const payloadRootEncryptionKey = crypto.ED25519ExtendedPrivateKey.fromPrivateKey(crypto.decodePrivateKey(payloadRootEncryptionKeyBytes));
            const payloadUriBytes = await messagingKey.ecdhDecrypt(crypto.decodePublicKey(ecdhKeyBundle.publicEphemeralKey), encryptedMessageUri);
