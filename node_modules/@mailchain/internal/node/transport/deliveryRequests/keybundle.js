...
async function createECDHKeyBundle(recipientMessagingKey, rand = crypto.secureRandom) {
    ...
    const payload = {
        publicMessagingKey: crypto.encodePublicKey(recipientMessagingKey),
        publicEphemeralKey: crypto.encodePublicKey(ephemeralKey.publicKey),
    };
    var errMsg = protocol.protocol.ECDHKeyBundle.verify(payload);
    if (errMsg) {
        throw Error(errMsg);
    }
    return {
