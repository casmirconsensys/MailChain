...
import { decodePublicKey, ED25519ExtendedPrivateKey, decodePrivateKey } from '@mailchain/crypto';
...
class DeliveryRequests {
    ...
    async processDeliveryRequest(messagingKey, delivery, deliveryRequestHash) {
        try {
            ...
            const payloadRootEncryptionKeyBytes = await messagingKey.ecdhDecrypt(decodePublicKey(ecdhKeyBundle.publicEphemeralKey), encryptedMessageKey);
            if (payloadRootEncryptionKeyBytes.length === 0) {
                throw new Error('payloadRootEncryptionKey is empty');
            }
            const payloadRootEncryptionKey = ED25519ExtendedPrivateKey.fromPrivateKey(decodePrivateKey(payloadRootEncryptionKeyBytes));
            const payloadUriBytes = await messagingKey.ecdhDecrypt(decodePublicKey(ecdhKeyBundle.publicEphemeralKey), encryptedMessageUri);
