...
import { ensureDecoded, EncodingTypes } from '@mailchain/encoding';
import { MailPreparer, MailDistributor } from '@mailchain/internal/browser/sending/mail';
import { MessagingKeys } from '@mailchain/internal/browser/messagingKeys';
import { toMailData } from '@mailchain/internal/browser/sending/mail/convertSendMailParams';
import { MailchainUserProfile } from '@mailchain/internal/browser/user';
import { MailchainMailboxOperations } from '@mailchain/internal/browser/mailbox';
import { defaultConfiguration } from '@mailchain/internal/browser/configuration';
...
class Mailchain {
    constructor(keyRing, config) {
        ...
        this._userProfile = MailchainUserProfile.create(config, keyRing.accountIdentityKey(), keyRing.userProfileCrypto());
        this._mailboxOperations = MailchainMailboxOperations.create(config, keyRing);
    }
    static fromAccountSeed(seed, config = defaultConfiguration) {
     ...
     */
    static fromSecretRecoveryPhrase(secretRecoveryPhrase, password, config = defaultConfiguration) {
     ...
     */
    static fromMnemonicPhrase(mnemonic, password, config = defaultConfiguration) {
        return Mailchain.fromSecretRecoveryPhrase(mnemonic, password, config);
    }
    static fromKeyRing(keyRing, config = defaultConfiguration) {
