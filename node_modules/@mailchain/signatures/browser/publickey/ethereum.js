...
import { addressFromPublicKey, ETHEREUM } from '@mailchain/addressing';
...
async function ethereumPublicKeyFromSignature(message, signature, expectedAddress) {
    ...
    if (!isEqual(address, expectedAddress)) {
        throw new Error(`inconsistent public key calculated, expected address "${encodeHex(expectedAddress)}" but actual is "${encodeHex(address)} (both hex encoded)"`);
    }
    return publicKey;
}

export { ethereumPublicKeyFromSignature };
